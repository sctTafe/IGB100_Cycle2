...
using UnityEngine;
// IK script referenced and modified from: https://github.com/lchaumartin/SpiderProceduralAnimation/tree/FirstVideo/Scripts and https://www.youtube.com/watch?v=swYBGqXtHEY&t=182s
...
public class SpiderProceduralAnimation : MonoBehaviour
{
    ...
    static Vector3[] MatchToSurfaceFromAbove(Vector3 point, float halfRange, Vector3 up)
    {
        ...
        if (Physics.Raycast(ray, out hit, 2f * halfRange, 1 << 13))
        {
            res[0] = hit.point;
            res[1] = hit.normal;
        }
    ...
    IEnumerator PerformStep(int index, Vector3 targetPoint)
    {
        ...
        for(float i = 0; i <= smoothness; i += Time.deltaTime)
        {
            legTargets[index].position = Vector3.Lerp(startPos, targetPoint, i / smoothness);
            legTargets[index].position += transform.up * Mathf.Sin(i / smoothness * Mathf.PI) * stepHeight;
